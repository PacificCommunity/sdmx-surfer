...
- Citations and provenance: when a dashboard displays SPC or partner data, the source needs to be cited per institutional conventions, including the .Stat URL and the date the data was retrieved. Partly delivered: each data source row can expand a published-provenance block (source, compiling organisation, compilation method, revision policy, licence, source link), fetched live from the gateway's `get_reference_metadata` and cached in process.
  ...
  Written against the 2026-08-03 gateway contract (`sdmx-mcp-gateway` PR #22), which reshaped this output: attributes attached below the dataflow now report themselves populated with no value and their text is fetched through the new `get_metadata_attribute`, resolved server-side so a panel still costs one request. Attributes a provider declares and leaves blank are reported explicitly and shown as such, since "this provider defines a licence field and published nothing in it" answers a licensing question. A value that merely held on every row a query returned is no longer presented as a statement about the dataset.
