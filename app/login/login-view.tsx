...
import type { OAuthProviderInfo } from "@/lib/oauth-providers";
...
function LoginForm({ providers }: { providers: OAuthProviderInfo[] }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  // Preserve callbackUrl from query params (e.g. from "Explore this" on a
  // published dashboard). Shared by the OAuth buttons and both form handlers.
    ...
    <div className="rounded-[var(--radius-xl)] bg-surface-card shadow-ambient ghost-border p-8">
      {sent ? (
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          {/* Offered first: for most people this is one click, while the email
              link is a round trip through an inbox. Rendered only for providers
              ...
              <div className="mb-5 mt-4 space-y-2">
                {providers.map((p) => (
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-surface-high px-4 py-2.5 text-sm font-medium text-on-surface transition-colors hover:bg-surface-high/70"
                  >
...
export function LoginView({ providers }: { providers: OAuthProviderInfo[] }) {
  return (
