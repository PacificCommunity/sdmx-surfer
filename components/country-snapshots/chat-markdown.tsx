...
export function ChatMarkdown({ text }: { text: string }) {
  return (
    <div className="chat-markdown space-y-2 text-sm [&_li]:ml-4 [&_li]:list-disc [&_ol_li]:list-decimal [&_code]:rounded [&_code]:bg-[#f1f4f6] [&_code]:px-1 [&_code]:font-mono [&_code]:text-xs [&_h1]:text-base [&_h1]:font-semibold [&_h2]:text-sm [&_h2]:font-semibold [&_h3]:text-sm [&_h3]:font-medium [&_table]:w-full [&_table]:text-xs [&_th]:px-2 [&_th]:py-1 [&_th]:text-left [&_th]:font-medium [&_td]:px-2 [&_td]:py-1 [&_thead]:bg-[#f7fafc]">
      <ReactMarkdown
        components={{
          a: ({ href, children }) => {
            const url = href ?? "";
            if (url.startsWith("/")) {
              return (
                <Link href={url} className="text-[#006970] underline">
                  {children}
                </Link>
              );
            }
